...
fn load_openclaw_config() -> Result<Value, String> {
    ...
    info!("[配置] 配置文件路径: {}", config_path);
    
    if !file::file_exists(&config_path) {
        info!("[配置] 配置文件不存在，返回空配置");
        return Ok(json!({}));
    }
    ...
    info!("[配置] 配置文件内容长度: {} bytes", content.len());
    if content.is_empty() {
        return Ok(json!({}));
    }
    
    // 过滤 JSON 中的反引号字符和 BOM 标记
    let clean_content: String = content
        .chars()
        .filter(|c| *c != '`' && *c != '\u{FEFF}')
        .collect();
    
    info!("[配置] 过滤后内容长度: {} bytes", clean_content.len());
    if clean_content.is_empty() {
        return Err("配置文件过滤后为空".to_string());
    }
    let preview = if clean_content.len() < 50 {
        clean_content.clone()
    } else {
        clean_content[..50].to_string()
    };
    info!("[配置] 过滤后内容预览: {}", preview);
    
    serde_json::from_str(&clean_content).map_err(|e| format!("解析配置文件失败: {}, 预览: {}", e, preview))
}
