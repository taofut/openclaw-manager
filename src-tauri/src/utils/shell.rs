...
pub fn get_extended_path() -> String {
    ...
    if let Some(home) = dirs::home_dir() {
        ...
        info!("[Shell] 检查 nvm default: {}", nvm_default);
        if let Ok(version) = std::fs::read_to_string(&nvm_default) {
            let version = version.trim();
            if !version.is_empty() {
                let nvm_path = format!("{}/.nvm/versions/node/v{}/bin", home_str, version);
                info!("[Shell] nvm 默认版本: {}, 路径: {}", version, nvm_path);
                paths.insert(0, nvm_path);
            }
        }
        
        // 常见 nvm 版本路径（检查存在的第一个）
        for version in ["v22.22.0", "v22.12.0", "v22.11.0", "v22.0.0", "v23.0.0"] {
            ...
            if std::path::Path::new(&nvm_bin).exists() {
                info!("[Shell] 找到 nvm node: {}", nvm_bin);
        ...
        // fnm
        let fnm_path = format!("{}/.fnm/aliases/default/bin", home_str);
        info!("[Shell] 检查 fnm: {}", fnm_path);
        if std::path::Path::new(&fnm_path).exists() {
            paths.push(fnm_path);
        }
        
        // volta
        let volta_path = format!("{}/.volta/bin", home_str);
        info!("[Shell] 检查 volta: {}", volta_path);
        if std::path::Path::new(&volta_path).exists() {
            paths.push(volta_path);
        }
        
        // asdf
        let asdf_path = format!("{}/.asdf/shims", home_str);
        info!("[Shell] 检查 asdf: {}", asdf_path);
        if std::path::Path::new(&asdf_path).exists() {
            paths.push(asdf_path);
        }
        
        // mise
        let mise_path = format!("{}/.local/share/mise/shims", home_str);
        info!("[Shell] 检查 mise: {}", mise_path);
        if std::path::Path::new(&mise_path).exists() {
            paths.push(mise_path);
        }
    }
    
    // 获取当前 PATH 并合并
    let current_path = std::env::var("PATH").unwrap_or_default();
    info!("[Shell] 当前 PATH 长度: {}", current_path.len());
    if !current_path.is_empty() {
        paths.push(current_path);
    }
    
    let result = paths.join(":");
    info!("[Shell] 扩展 PATH 构建完成，总长度: {}", result.len());
    result
}
...
/// 查找 openclaw.mjs 的直接路径（绕过锁机制）
fn find_openclaw_mjs_path() -> Option<String> {
    let possible_paths = if platform::is_windows() {
        let mut paths = Vec::new();
        
        // 用户目录下的 npm 全局路径
        if let Some(home) = dirs::home_dir() {
            paths.push(format!("{}\\AppData\\Roaming\\npm\\node_modules\\openclaw\\openclaw.mjs", home.display()));
        }
        ...
        // 用户目录下的 npm 全局路径
        if let Some(home) = dirs::home_dir() {
            paths.push(format!("{}/.npm-global/lib/node_modules/openclaw/openclaw.mjs", home.display()));
            paths.push(format!("{}/.nvm/versions/node/*/lib/node_modules/openclaw/openclaw.mjs", home.display()));
        }
        
        // 全局 node_modules
        paths.push("/usr/local/lib/node_modules/openclaw/openclaw.mjs".to_string());
    ...
    for path in possible_paths {
        if std::path::Path::new(&path).exists() {
            info!("[Shell] 找到 openclaw.mjs: {}", path);
            return Some(path);
        }
    }
    
    // 尝试通过 npm root 命令查找
    if let Ok(npm_root) = run_npm_root() {
        let mjs_path = format!("{}/openclaw/openclaw.mjs", npm_root);
        if std::path::Path::new(&mjs_path).exists() {
            info!("[Shell] 通过 npm root 找到 openclaw.mjs: {}", mjs_path);
...
fn run_npm_root() -> Result<String, String> {
    ...
    match output {
        ...
        _ => Err("npm root -g failed".to_string())
    }
}
...
fn load_openclaw_env_vars() -> HashMap<String, String> {
    ...
    info!("[Shell] 尝试读取环境变量文件: {}", env_path);
    
    if let Ok(content) = file::read_file(&env_path) {
        info!("[Shell] 成功读取环境变量文件, 内容长度: {}", content.len());
...
pub fn spawn_openclaw_gateway() -> io::Result<()> {
    ...
    // 查找 openclaw.mjs 的直接路径
    let openclaw_mjs_path = find_openclaw_mjs_path();
    if openclaw_mjs_path.is_some() {
        info!("[Shell] 找到 openclaw.mjs");
    }
    ...
    // 使用 openclaw gateway 命令启动
    let mut cmd = if openclaw_path.ends_with(".cmd") {
        info!("[Shell] Windows 模式: 使用 cmd /c 执行 openclaw gateway");
    ...
    match child {
        Ok(mut c) => {
            let pid = c.id();
            info!("[Shell] ✓ Gateway 进程已启动, PID: {}", pid);
            ...
            if let Ok(Some(_)) = c.try_wait() {
                warn!("[Shell] ✗ Gateway 进程启动后立即退出，请检查 gateway.err.log");
                // 读取错误日志
                let err_log = format!("{}/logs/gateway.err.log", platform::get_config_dir());
                if let Ok(content) = std::fs::read_to_string(&err_log) {
                    let lines: Vec<&str> = content.lines().collect();
                    let last_lines = lines.last().unwrap_or(&"");
                    info!("[Shell] gateway.err.log 最后一行: {}", last_lines);
                }
            } else {
                info!("[Shell] Gateway 进程仍在运行");
            }
